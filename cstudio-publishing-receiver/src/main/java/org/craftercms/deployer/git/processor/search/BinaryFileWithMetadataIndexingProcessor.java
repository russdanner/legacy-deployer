...
/**
 * {@link org.craftercms.deployer.git.processor.PublishingProcessor} that tries to match the update/deleted files
 * against a series of metadata/binary path patterns. If a file is matched as metadata, it's is parsed for a reference
 * to it's binary. If one is found, then an update will be issued for the binary file along with the metadata that is
 * extracted from the metadata file, or a delete for the referenced binary file. If the file is matched as binary,
 * then an update/delete is issued without any metadata.
 *
 * @author avasquez
 */
public class BinaryFileWithMetadataIndexingProcessor extends AbstractIndexingProcessor {

    private static final Log logger = LogFactory.getLog(BinaryFileWithMetadataIndexingProcessor.class);
    ...
    protected int update(String indexId, String siteName, String root, List<String> fileNames,
        ...
        for (String fileName : fileNames) {
            ...
            if (isMetadataFile(fileName)) {
                ...
                try {
                    ...
                    if (StringUtils.isNotBlank(updateFileName)) {
                        updateFile = new File(root, updateFileName);

                        if (logger.isDebugEnabled()) {
                            logger.debug("Binary file for metadata file " + file + ": " + updateFile);
                        }

                        metadataDoc = processDocument(metadataDoc, file, root);
            ...
            if (doUpdate) {
                if (!delete) {
                    doUpdateFile(indexId, siteName, updateFileName, updateFile, metadata);
                    updateCount++;
                } else {
                    doDelete(indexId, siteName, updateFileName, updateFile);
                    updateCount++;
                }
            }
        }
